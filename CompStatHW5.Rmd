...
Since in the question $k=3$ and $n=3$, there are $(k!)^{n-1} = (3!)^2 = 36$ total permutations that need to be checked.  The first row does not need to be permuted.  If we were to permute the first row, it would just lead to six duplicates of each test statistic calucated by leaving the first row fixed.  

We do not have duplicate values in the rows and do not need to take this issue into account.  The code below calculates the permutations of the second and third rows, and then iterates through all combinations and calcuates the test statistic for each case.
...
Finally, we can calculate what proportion of the test statistics have a value greater than the original data set's:
...
The p-value is $`r p_value`$.  This is because in the data set, Treatment C contains the three highest values in each row, Treatment B contains the second largest values in each row, and Treatment A contains the smallest number in each row.  There is no other arrangement of data that produces a larger sum of squared column totals.  Thus we reject the null hypothesis and conclude that the population means of the three treatments are not all equal to each other.
